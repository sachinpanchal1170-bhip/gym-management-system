...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($oldPassword) || empty($newPassword)) {
        ...
        $stmt = $con->prepare("SELECT id FROM trainers WHERE email = ? AND password = ?");
        ...
        if ($result->num_rows === 1) {
            ...
            $updateStmt = $con->prepare("UPDATE trainers SET password = ? WHERE id = ?");
...
<body>
    ...
    <form method="POST" action="">
        ...
        <button type="button" onclick="window.location.href='trainer_login.php'">Back</button>
    </form>
